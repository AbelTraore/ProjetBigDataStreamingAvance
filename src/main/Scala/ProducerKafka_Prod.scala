import KafkaStreaming.{getJSON, getKafkaProducerParams_exactly_once}
...
object ProducerKafka_Prod {
  ...
  def ProducerKafka_prod (KafkaBootStrapServers : String, topic_name : String) : Unit = {
    ...
    try {
      ...
      producer_Kafka.send(record_publish, new Callback {
        override def onCompletion(recordMetadata: RecordMetadata, e: Exception): Unit = {
          if (e == null) {
            ...
            println(" offset du message" + recordMetadata.offset().toString())
            println(" topic du message" + recordMetadata.topic().toString())
            println(" partition du message" + recordMetadata.partition().toString())
            println(" heure d'enregistrement du message" + recordMetadata.timestamp())
      ...
      //trace_kafka.info(s"La liste des paramètres pour la connexion du producer kafka sont : ${getKafkaProducerParams_prod(KafkaBootStrapServers)}")
    }
