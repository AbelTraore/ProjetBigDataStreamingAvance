import ConsommationStreaming.{bootStrapServers, consumerGroupId, consumerReadOrder, kerberosName, topics, zookeeper}
...
val schema_indicateurs = StructType (Array(
  ...
  private val path_fichiers_kpi : String = "/data lake/marketing_AT/Projet Streaming/kpi"   //"D:\\Dev_appli_streaming_avancees_juv\\kpi"

  private var loger : Logger = LogManager.getLogger("Log_Console")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val kk_consumer = getConsommateurKafka(bootStrapServers, consumerGroupId, consumerReadOrder,
    ...
    kk_consumer.foreachRDD {
        ...
        rdd_kafka => {
          ...
          if (events_df.count() == 0) {
            ...
            val log_events = Seq(
              "Aucun événement n'a été réceptionné dans le quart d'heure "
            ).toDF("libellé")
              .coalesce(1)
              .write
              ...
              .save(path_fichiers_kpi + "/indicateurs.csv")

          } else {

            val df_parsed = getParseData(events_df, ssession)
          ...
          case ex: Exception => loger.error(s"il y a une erreur dans l'application ${ex.printStackTrace()}")
  ...
  def getParseData (kafkaEventsDf : DataFrame, ss : SparkSession) : DataFrame = {
    ...
    loger.info("parsing de json reçus de kafka encours...")

    import  ss.implicits._

    val def_events = kafkaEventsDf.withColumn("kafka_jsons", from_json(col("kafka_jsons"), schema_indicateurs))
  ...
  def getIndicateursComputed (eventsDf_parsed : DataFrame, ss : SparkSession) : DataFrame = {
    ...
    loger.info("calcul des indicateurs encours....")

    import ss.implicits._

    eventsDf_parsed.createOrReplaceGlobalTempView("events_tweets")
